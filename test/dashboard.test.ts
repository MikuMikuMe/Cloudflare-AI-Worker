...
test('dashboard exposes persistent Chats navigation and conversation API actions', () => {
  ...
  assert.match(dashboard, /provider_fallback/);
  assert.match(dashboard, /Continued with NVIDIA/);
  assert.match(dashboard, /id="provider-fallback" type="checkbox"/);
  assert.match(dashboard, /selected\.value\.indexOf\('@cf\/'\) === 0/);
  assert.match(dashboard, /if \(!hasMappedFallback\) checkbox\.checked = false/);
  assert.match(dashboard, /var requestedModel = \$\('#model'\)\.value/);
  assert.match(dashboard, /allow_provider_fallback: allowProviderFallback/);
  assert.match(dashboard, /Enable NVIDIA backup or choose an NVIDIA model\./);
  assert.match(dashboard, /var previous = pendingModel \|\| sel\.value \|\| \(activeConversation && activeConversation\.last_model\)/);
  assert.match(dashboard, /if \(saved\) \{/);
  assert.match(dashboard, /else if \(defaultCloudflare\)/);
  assert.match(dashboard, /Paid billing required/);
  ...
  assert.doesNotMatch(dashboard, /temporaryFallback/);
  assert.doesNotMatch(dashboard, /recommendedNvidiaOption/);
  assert.doesNotMatch(dashboard, /selectTemporaryProviderFallback/);
  assert.doesNotMatch(dashboard, /clearTemporaryProviderFallback/);
